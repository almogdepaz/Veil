...
fn process_coin(
    ...
    let (instance_bytecode, program_hash, function_table) =
        compile_chialisp_to_bytecode_with_table(sp1_hasher, chialisp_source, program_parameters)
        ...
        match condition.opcode {
            62 => {
                ...
                if !condition.args.is_empty() {
                    ...
                    puzzle_announcement_hashes.push(sp1_hasher(&data));
                }
            }
            60 => {
                ...
                if !condition.args.is_empty() {
                    if let Some(cc) = coin_commitment {
                        ...
                        data.extend_from_slice(message);
                        coin_announcement_hashes.push(sp1_hasher(&data));
                    }
                }
            }
            _ => {}
