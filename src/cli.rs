    ...
    for (name, expression, parameters) in demos {
        ...
        match ClvmZkProver::prove(expression, &parameters) {
            Ok(proof_result) => {
                ...
                println!(
                    "   Output matches: {}",
                    output == proof_result.clvm_res.output
                );

                match ClvmZkProver::verify_proof(
                    program_hash,
                    &proof_result.proof,
                    Some(&proof_result.clvm_res.output),
                ) {
...
fn run_prove(
    ...
    match ClvmZkProver::prove(&expression, &parameters) {
        Ok(proof_result) => {
            ...
            let output_bytes = &proof_result.clvm_res.output;
            ...
            println!("Cost: {}", proof_result.clvm_res.cost);
