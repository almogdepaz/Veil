...
if [ -f "./install-deps.sh" ]; then
    ...
    echo "⚠️  risc0 dependencies not available - run ./install-deps.sh"
fi
...
if command -v cargo-prove &> /dev/null; then
    ...
    echo "⚠️  sp1 toolchain not available - run 'curl -L https://sp1.succinct.xyz | bash && sp1up'"
fi
...
if command -v docker &> /dev/null; then
    if docker info &> /dev/null; then
        ...
        echo "⚠️  docker installed but not running - plonk/groth16 modes will be skipped"
        echo "    start docker: 'open -a Docker' (macOS) or 'sudo systemctl start docker' (linux)"
        DOCKER_AVAILABLE=false
    fi
else
    echo "⚠️  docker not installed - plonk/groth16 modes will be skipped"
...
if [ "$DOCKER_AVAILABLE" = true ]; then
    ...
    echo
    echo "⏭️  skipping plonk and groth16 modes (docker required)"
fi
