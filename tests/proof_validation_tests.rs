...
fn test_proofs_differ_for_different_inputs() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output1 = proof_result1.clvm_res;
    ...
    let output2 = proof_result2.clvm_res;
    ...
    println!("DEBUG: 'a' alone = {:?}, expected = [42]", test_a.clvm_res);
        ...
        let futures: Vec<_> = batch
            ...
            .map(|(dividend, divisor, expected_quot, expected_rem)| {
                ...
                task::spawn_blocking(move || -> Result<_, String> {
                    ...
                    if output.output != expected_bytes {
                        return Err(format!(
