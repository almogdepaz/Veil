...
#[cfg(feature = "mock")]
mod tests {
    use clvm_zk::payment_keys::{PaymentKey, derive_ecdh_puzzle_hash};
    ...
    fn test_scan_finds_ecdh_payment() {
        ...
        // taker creates payment to maker via ecdh
        let taker_key = PaymentKey::generate();
        let payment_puzzle_hash = derive_ecdh_puzzle_hash(
            &maker_key.to_pubkey(),
            &taker_key.to_pubkey(),
        );
        ...
        let coins = vec![
            (other_coin1, [0x02; 33]),           // not ours
            (payment_coin, taker_key.to_pubkey()), // THIS ONE - ecdh match
            (other_coin2, [0x03; 33]),           // not ours
        ];
    ...
    fn test_scan_multiple_payments() {
        ...
        // create ecdh coins from sender1 and sender3
        let payment1_puzzle = derive_ecdh_puzzle_hash(&receiver.to_pubkey(), &sender1.to_pubkey());
        let payment2_puzzle = derive_ecdh_puzzle_hash(&receiver.to_pubkey(), &sender3.to_pubkey());
    ...
    fn test_scan_with_pubkey_only_returns_empty() {
        ...
        let pubkey_only = PaymentKey::from_pubkey([0x02; 33]);

        let coin = PrivateCoin::new_with_secrets([0x11; 32], 1000).0;
        let coins = vec![(coin, [0x03; 33])];
    ...
    fn test_can_spend_ecdh_coin_verification() {
        ...
        // correct ecdh derivation
        let correct_puzzle = derive_ecdh_puzzle_hash(&receiver.to_pubkey(), &sender.to_pubkey());
        assert!(
